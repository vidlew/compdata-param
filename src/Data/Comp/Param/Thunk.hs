...
-- found.
whnf :: (Monad m, Thunk m :<: (Thunk m :+: f)) => TrmT m f a -> m (Either a (f a (TrmT m f a)))
whnf (In (Inl (Thunk m))) = m >>= whnf
whnf (In (Inr t)) = return $ Right t
whnf (Var x) = return $ Left x

whnf' :: (Monad m, f :<: (Thunk m :+: f), Thunk m :<: (Thunk m :+: f)) => TrmT m f a -> m (TrmT m f a)
...
whnfPr :: (MonadFail m, g :<: f, Thunk m :<: (Thunk m :+: f)) => TrmT m f a -> m (g a (TrmT m f a))
...
nfT :: (ParamFunctor m, Monad m, Ditraversable f, Thunk m :<: (Thunk m :+: f)) => TermT m f -> m (Term f)
nfT t = termM $ nf $ unTerm  t

-- | This function evaluates all thunks.
nf :: (Monad m, Ditraversable f, Thunk m :<: (Thunk m :+: f)) => TrmT m f a -> m (Trm f a)
...
nfTPr :: (ParamFunctor m, MonadFail m, Ditraversable g, g :<: f, Thunk m :<: (Thunk m :+: f)) => TermT m f -> m (Term g)
nfTPr t = termM $ nfPr $ unTerm t

-- | This function evaluates all thunks while simultaneously
-- projecting the term to a smaller signature. Failure to do the
-- projection is signalled as a failure in the monad as in 'whnfPr'.
nfPr :: (MonadFail m, Ditraversable g, g :<: f, Thunk m :<: (Thunk m :+: f)) => TrmT m f a -> m (Trm g a)
nfPr = liftM In . dimapM nfPr <=< whnfPr


evalStrict :: (Ditraversable g, Monad m, g :<: f, Thunk m :<: (Thunk m :+: f), g :<: (Thunk m :+: f)) => 
...
strict :: (f :<: g, Ditraversable f, Monad m, f :<: (Thunk m :+: g), Thunk m :<: (Thunk m :+: g), g :<: (Thunk m :+: g)) => f a (TrmT m g a) -> TrmT m g a
strict x = thunk $ liftM inject $ dimapM whnf' x

-- | This combinator makes the evaluation of the given functor
-- application strict by evaluating all thunks of immediate subterms.
strict' :: (f :<: g, Ditraversable f, Monad m, f :<: (Thunk m :+: g), Thunk m :<: (Thunk m :+: g), g :<: (Thunk m :+: g)) => f (TrmT m g a) (TrmT m g a) -> TrmT m g a
