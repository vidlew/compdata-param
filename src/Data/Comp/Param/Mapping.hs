...
module Data.Comp.Param.Mapping
    ( Numbered (..)
    , unNumbered
    , number
    , Traversable ()
...
data Numbered a = Numbered Int a deriving Functor
...
number :: Ditraversable f => f a b -> f a (Numbered b)
number x = evalState (dimapM run x) 0 where
  run b = do n <- get
             put (n+1)
             return $ Numbered n b
...
newtype NumMap k v = NumMap (IntMap v) deriving (Foldable,Traversable)
instance Difunctor NumMap where
  dimap _ g (NumMap m) = NumMap $ fmap g m
